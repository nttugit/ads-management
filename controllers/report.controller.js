...
import LocationReportHandler from '../handlers/locationReport.handler.js';
...
const locationReportHandler = new LocationReportHandler();

import { reverseGeocodingWithForwardReverse2 } from '../utils/map.js';
...
controller.patchAdsReport = async (req, res) => {
    ...
    // return res.status(400).json(req.staff);
...
// ========================= LOCATION REPORT =========================
controller.getLocationReports = async (req, res) => {
    const {
        size = 50,
        page = 1,
        districts = [],
    ...
    const data = await locationReportHandler.getList(
        conditions,
        {},
        pagination,
        populate,
    );
    const totalItems = await locationReportHandler.count(conditions);
    // const {size}
    res.status(200).json(
        RESPONSE.SUCCESS(data, 'get sucessfully', {
            pagination: {
...
controller.getLocationReport = async (req, res) => {
    // id: id của ads report
    const { id } = req.params;
    const populate = [
        {
    ...
    const adsReport = await locationReportHandler.getById(id, {}, populate);
    // Nhớ return khi muốn kết thúc
    if (!adsReport) return res.status(204).send();
    res.status(200).json(RESPONSE.SUCCESS(adsReport, 'get sucessfully'));
};

controller.postLocationReport = async (req, res) => {
    /**
     * 1. Dữ liệu gửi lên: toạ độ, nội dung báo cáo (bao gồm hình ảnh)
     * 2. Tạo report, Cập nhật Location Report và Image
     * 3. Thêm phường/quận cho report luôn cho tiện truy vấn kkk
     */
    // Todo: validate
    const {
    ...
    let wardName = null;
    let existDistrict = null;
    let existWard = null;
    const geocodeError =
        'Địa điểm này không nằm trong khu vực quản lý hoặc hệ thống xác định vị trí bị sai';

    const { display_name } = await reverseGeocodingWithForwardReverse2(
        lat,
        long,
    );
    if (display_name) {
        if (display_name.includes('Phường')) {
            // Split the string by commas and get the first part
            let parts = display_name.split(',');

            // Find the part that contains "Phường" and extract the ward
            let wardPart = parts.find((part) => part.includes('Phường'));
            wardName = wardPart.split('Phường')[1].trim();

            let districtPart = parts.find((part) => part.includes('Quận'));
            districtName = districtPart.split('Quận')[1].trim();

            console.log('Ward:', wardName);
            console.log('district:', districtName);
        } else {
            return res.status(400).json(RESPONSE.FAILURE(400, geocodeError));
        }
    } else {
        return res.status(400).json(RESPONSE.FAILURE(400, geocodeError));
    }

    existDistrict = await districtHandler.getOne({
        name: { $regex: new RegExp(`${districtName}`, 'i') },
    });
    if (existDistrict) {
        existWard = await wardHandler.getOne({
    ...
    // const ward = ads.adsLocation.address.ward;
    // const district = ads.adsLocation.address.district;
    const [updateImage, newLocationReport] = await Promise.all([
        imageHandler.updateMany(
            { _id: { $in: imageIds } },
            {
                report: newReport._id,
    ...
    if (newLocationReport) {
        const reportSolution = await reportSolutionHandler.getOne({
            ward: existWard._id,
        });
        const content = createReportEmailContent(
            fullName,
            ...
            existDistrict.name,
            reportSolution?.solution ||
                'Cán bộ đang xác minh thông tin. Nếu có vi phạm lập tức xử lý',
        );
        await sendEmail(email, EMAIL_TITLES.LOCATION, content);
    }
    ...
    res.status(201).json(
        RESPONSE.SUCCESS(
            {
                report: newReport,
                locationReport: newLocationReport,
            },
            'create successfully',
        ),
    );
